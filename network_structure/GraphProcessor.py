...
class GraphProcessor(object):
    def __init__(self, graphNetwork):
        ...
        self.second_bigest_val, self.second_bigest_left_vector, self.second_bigest_right_vector, self.lambda_0 = self.row_second_bigest_eigenvalue(self.base_graph)
        ...
        if graphNetwork.args.method == 'xuni':
        ...
        # print(f"真实网络的行归一矩阵在第 {self.find_stable_power(self.base_row_w)} 轮次收敛")
        # print(f"虚拟网络的行归一矩阵在第 {self.find_stable_power(self.row_w)} 轮次收敛")
    ...
    def row_second_bigest_eigenvalue(self, graph):
        ...
        return second_bigest_eigenvalue, second_bigest_left_vector, second_bigest_right_vector, real_eigenvalues[smallest_index]
    ...
    def max_independent_set(self):
        ...
        # 获取图的节点数，即邻接矩阵的行数或列数
        # n = self.independent_matrix.shape[0]
        ...
        while True:
            ...
            # 计算行归一化矩阵
            row_matrix = self.generate_row_weight_matrix(virtual_graph)
            ...
            virtual_value = real_eigenvalues[second_bigest_index]
        ...
        # 保存特征值变化的曲线
        print("保存特征值变化的曲线")
        file_path = f"{self.graphNetwork.args.size}_{self.graphNetwork.args.randomSeed}_{self.graphNetwork.args.method}_3_fast.txt"
        try:
    ...
    def generate_virtual_graph(self):
        ...
        virtual_value, virtual_left_vec, virtual_right_vec, self.lambda_n = self.row_second_bigest_eigenvalue(virtual_graph)
